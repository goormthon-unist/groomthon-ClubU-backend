...
def test_production_mypage():
    ...
    try:
        response = session.post(f"{BASE_URL}/api/v1/auth/register", json=register_data, timeout=30)
    ...
    try:
        response = session.post(f"{BASE_URL}/api/v1/auth/login", json=login_data, timeout=30)
