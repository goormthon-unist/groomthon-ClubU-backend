...
def get_clubs():
    ...
    try:
        # 동아리와 카테고리 정보를 함께 조회
        clubs = db.session.query(Club, ClubCategory).join(
            ClubCategory, Club.category_id == ClubCategory.id
        ).all()
...
def get_club_detail(club_id):
    """특정 동아리 상세 정보 조회"""
    try:
        club = db.session.query(Club, ClubCategory).join(
            ClubCategory, Club.category_id == ClubCategory.id
        ...
        club_data = {
            ...
            'created_at': club_obj.created_at.isoformat() if club_obj.created_at else None,
            'updated_at': club_obj.updated_at.isoformat() if club_obj.updated_at else None
        }
