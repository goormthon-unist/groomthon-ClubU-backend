    ...
    def get_room_availability(room_id: int, target_date: str) -> Dict:
        ...
        for reservation in reservations:
            booked_slots.append({
                ...
                "club_name": reservation.club.name if reservation.club else "알 수 없음",
        ...
        for hour in range(start_hour, end_hour):
            ...
            is_booked = False
            for reservation in reservations:
                if (reservation.start_time <= slot_start < reservation.end_time or
                    reservation.start_time < slot_end <= reservation.end_time or
                    (reservation.start_time <= slot_start and reservation.end_time >= slot_end)):
                    is_booked = True
                    break
    ...
    def get_club_remaining_usage(club_id: int, target_date: str) -> Dict:
        ...
        for reservation in reservations:
            ...
            reservation_details.append({
                "id": reservation.id,
                "room_name": reservation.room.name if reservation.room else "알 수 없음",
                "start_time": reservation.start_time.strftime("%H:%M"),
                "end_time": reservation.end_time.strftime("%H:%M"),
