...
def change_club_member_role(
    ...
    try:
        ...
        if role_name not in allowed_roles:
            raise ValueError(f"동아리 내에서 허용되지 않는 역할입니다. 허용된 역할: {', '.join(allowed_roles)}")
...
def get_club_members_list(club_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        memberships = ClubMember.query.filter_by(club_id=club_id).join(
            User, ClubMember.user_id == User.id
        ).join(
            Role, ClubMember.role_id == Role.id
        ).order_by(
